	    ...
	    P::Target: Persist<ChannelSigner>,
{
	...
	{
			{
					let _monitor_states = self.monitors.write().unwrap();
			...
			for funding_outpoint in funding_outpoints.iter() {
					...
					if let Some(monitor) = monitor_states.get(funding_outpoint) {
							self.update_monitor(header, best_height, txdata, &process, funding_outpoint, &monitor);
					}
			}

			{
					let monitor_states = self.monitors.write().unwrap();
					for (funding_outpoint, monitor) in monitor_states.iter() {
	...
	fn update_monitor<FN>(&self, header: &BlockHeader, best_height: Option<u32>, txdata: &TransactionData, process: FN, funding_outpoint: &OutPoint, monitor_state: &&MonitorHolder<ChannelSigner>) where FN: Fn(&ChannelMonitor<ChannelSigner>, &TransactionData) -> Vec<TransactionOutputs> {
